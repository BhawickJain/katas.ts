...
test("countOccurancesZero returns an array of char-count pairs", () => {
  expect(countOccurancesZero("hello")).toStrictEqual([
    ["h", 1],
    ["e", 1],
    ["l", 2],
    ["o", 1],
  ]);
  expect(countOccurancesZero("mississippi")).toStrictEqual([
    ["m", 1],
    ["i", 4],
    ["s", 4],
    ["p", 2],
  ]);
  expect(countOccurancesZero("necessary")).toStrictEqual([
    ["n", 1],
    ["e", 2],
    ["c", 1],
    ["s", 2],
  ...
  expect(countOccurancesZero("wass'up")).toStrictEqual([
    ["w", 1],
    ["a", 1],
    ["s", 2],
    ["'", 1],
...
implementations.forEach((imp) => {
  test(`${imp.name} returns an array of char-count pairs`, () => {
    ...
    expect(imp("necessary")).toStrictEqual({
      n: 1,
      e: 2,
      c: 1,
      s: 2,
    ...
    expect(imp("wass'up")).toStrictEqual({
      w: 1,
      a: 1,
      s: 2,
      "'": 1,
