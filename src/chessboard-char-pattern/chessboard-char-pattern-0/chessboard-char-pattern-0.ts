...
export function chessboardCharacterPattern(chessboardRequest: string): void | String {
    ...
    const arrayOfChessboardStringRequests: string[] = arrayOfInstructions.slice(1)
	...
	const arrayOfChessboardRequests: chessboardRequest[] = getArrayOfPatternRequests(arrayOfChessboardStringRequests)
    // console.log(arrayOfChessboardRequests)
	const arrayOfStringChessBoards: string[][] =   arrayOfChessboardRequests.map(generateChessStringPattern)

	printChessBoard(arrayOfStringChessBoards)
}

function getNumOfPatternsFromRequest(arrayOfChessboardStringRequests: string[]): number {

	let numTestCases: number | string | undefined = arrayOfChessboardStringRequests[0]
...
export function getArrayOfPatternRequests(arrayOfPatternsRequests: string[]): chessboardRequest[] {
    const arrayOfChessboardRequests: chessboardRequest[] = arrayOfPatternsRequests.map(
        pattern => createChessboardRequest(pattern)
    )
...
function createChessboardRequest(pattern: string): chessboardRequest {
    ...
    if (stringArrayChessboardRequest.length === 2) {
        const intChessboardRequest: number[] = stringArrayChessboardRequest.map(str => parseInt(str))
...
export function generateChessStringPattern(chessboardRequestInput: chessboardRequest): string[] {
...
export function generateCheckerPattern(patternList: [string, string], length: number): string {
  
  let patternjoined: string = patternList.join('')
  let remainder: number = length % patternList.length
  let multiple: number = (length - remainder) / patternList.length
  let remainingPattern = patternjoined.slice(0, remainder)
...
function printChessBoard(arrayOfStringChessBoards: string[][]): void {
	arrayOfStringChessBoards.forEach( chessboard => {
		console.log('\n')
		chessboard.forEach(row => {console.log(row)})
	})
}
